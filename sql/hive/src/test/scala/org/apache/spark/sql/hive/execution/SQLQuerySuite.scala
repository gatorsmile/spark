...
class SQLQuerySuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
  ...
  test("saveAsTable(CTAS) when the source DataFrame is built on a Hive table") {
    ...
    withTable(tableName) {
      ...
      df1.write.mode("append").saveAsTable(tableName)
      checkAnswer(
        sql(s"SELECT key, value FROM $tableName"),
      ...
      df2.write.mode("append").saveAsTable(tableName)
      checkAnswer(
        sql(s"SELECT key, value FROM $tableName"),
      ...
      val e = intercept[AnalysisException] {
        df3.write.mode("append").saveAsTable(tableName)
      }.getMessage
      assert(e.contains("Unable to resolve key given [value, non_existent]"))
    }
  }
