 ...
 */
private[hive]
case class HiveRelationScanExec(
    @transient private val catalogTable: CatalogTable,
    requestedAttributes: Seq[Attribute],
  ...
  require(partitionPruningPred.isEmpty || catalogTable.isPartitioned,
    "Partition pruning predicates only supported for partitioned tables.")
  ...
  private[this] val boundPruningPred = partitionPruningPred.reduceLeftOption(And).map { pred =>
    ...
    BindReferences.bindReference(pred, requestedPartitionAttributes)
  }
  ...
  val columnOrdinals: Map[String, Int] = {
    val attributes: Seq[String] = catalogTable.schema
      .filter { c => !catalogTable.partitionColumnNames.contains(c.name) }
      .map(_.name)
    attributes.zipWithIndex.toMap
  }
  ...
  private[this] val hadoopReader =
    new HiveTableReader(
      ...
      tableDesc = tableDesc,
      partitionKeys = requestedPartitionAttributes,
  ...
  private def addColumnMetadataToConf(hiveConf: Configuration) {
    // Specifies needed column IDs for those non-partitioning columns.
    val neededColumnIDs =
      requestedAttributes.flatMap(a => columnOrdinals.get(a.name)).map(o => o: Integer)

    HiveShim.appendReadColumns(hiveConf, neededColumnIDs, requestedAttributes.map(_.name))
    ...
    hiveConf.set(serdeConstants.LIST_COLUMN_TYPES, columnTypeNames)
    hiveConf.set(
      serdeConstants.LIST_COLUMNS, catalogTable.schema.map(_.name).mkString(","))
  }
  ...
  private[hive] def prunePartitions(partitions: Seq[HivePartition]) = {
    boundPruningPred match {
      case None => partitions
      case Some(shouldKeep) => partitions.filter { part =>
        val dataTypes = requestedPartitionAttributes.map(_.dataType)
        val castedValues = part.getValues.asScala.zip(dataTypes)
          .map { case (value, dataType) => castFromString(value, dataType) }
  ...
  protected override def doExecute(): RDD[InternalRow] = {
    // Using dummyCallSite, as getCallSite can turn out to be expensive with
    // with multiple partitions.
    val rdd = if (!catalogTable.isPartitioned) {
      Utils.withDummyCallSite(sqlContext.sparkContext) {
        hadoopReader.makeRDDForTable(hiveQlTable)
      }
    } else {
      val partitions =
        HiveUtils.getHiveQlPartitions(sparkSession, catalogTable, partitionPruningPred)
      Utils.withDummyCallSite(sqlContext.sparkContext) {
        hadoopReader.makeRDDForPartitionedTable(
          prunePartitions(partitions))
      }
